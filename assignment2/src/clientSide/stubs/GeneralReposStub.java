...
//	}
	...
	public void setStudentState(int id, int state, boolean hold)
	{
		...
		if(inMessage.getMsgType() != MessageType.SACK2)
		{
			GenericIO.writelnString ("Thread " + Thread.currentThread ().getName () + ": Invalid message type!");
			GenericIO.writelnString (inMessage.toString ());
			System.exit (1);
	...
	public void setWaiterState(int state) {
		...
		com.writeObject(outMessage);
		inMessage = (Message) com.readObject();
		if (inMessage.getMsgType() != MessageType.WSTDONE) {
			GenericIO.writelnString("Thread " + Thread.currentThread().getName() + ": Invalid message type!");
			GenericIO.writelnString(inMessage.toString());
			System.exit(1);
		}
	...
	public void setnCourses(int nCourses)
	{
		...
		//Validate inGoing message type and arguments
		if(inMessage.getMsgType() != MessageType.COURDONE)
		{
			GenericIO.writelnString ("Thread " + Thread.currentThread ().getName () + ": Invalid message type!");
			GenericIO.writelnString (inMessage.toString ());
			System.exit (1);
	...
	public void updateSeatsAtTable(int studentID, int studentSeat) {
		...
		if (inMessage.getMsgType() != MessageType.SATDONE/* MessageType.USSEATDONE */) {
			GenericIO.writelnString("Thread " + Thread.currentThread().getName() + ": Invalid message type!");
			GenericIO.writelnString(inMessage.toString());
			System.exit(1);
		}
	...
	public void updateSeatsAtLeaving(int id)
	{
		...
		//Validate inGoing message type and arguments
		if(inMessage.getMsgType() != MessageType.ATLDONE)
		{
			GenericIO.writelnString ("Thread " + Thread.currentThread ().getName () + ": Invalid message type!");
			GenericIO.writelnString (inMessage.toString ());
			System.exit (1);
